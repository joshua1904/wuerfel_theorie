...
def add_values(arr_1: list, arr_2):
...
def run_game(current_game_state: dict, possibilities: list, already_won):
    ...
    for key, value in tqdm(current_game_state.items()):
        #wandelt den dict key zu einem int array um
        ...
        for i in possibilities:
            ...
            if not is_win(new_state):
                ...
                new_state_key = "-".join([str(i) for i in new_state])
                if new_state_key not in game_dict:
                    game_dict[new_state_key] = value
                else:
                    game_dict[new_state_key] += value
            else:
                wins += value
